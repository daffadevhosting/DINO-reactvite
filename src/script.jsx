...
import { CSS3DRenderer, CSS3DObject } from 'three/examples/jsm/renderers/CSS3DRenderer.js';
    ...
    const metadata = [
				'/textures/img/0001.png', 'Iridium', '0.02 ETH', 1, 1,
...
let canvas, camera, scene, renderer, href;
let controls;
let isUserInteracting = true;
    const objects = [];
    const tujuan = { metadata: [], grid: [] };
...
for ( let i = 0; i < metadata.length; i += 5 ) {
    ...
    const element = document.createElement( 'div' );
                    ...
                    element.style.touchAction = 'true';
    ...
    const details = document.createElement( 'div' );
					...
					details.innerHTML = metadata[ i + 1 ] + '<br>' + metadata[ i + 2 ];
					element.appendChild( details );

    const trigger = document.createElement( 'a' );
                    trigger.href = '/token/0xFfd9bAddF3f6e427EfAa1A4AEC99131078C1d683';
                    trigger.id = 'buttonBuy';
                    trigger.role = 'button';
    ...
    const object = new THREE.Object3D();
					object.position.x = ( metadata[ i + 3 ] * 240 ) - 1140;
					object.position.y = - ( metadata[ i + 4 ] * 280 ) + 990;

					tujuan.metadata.push( object );
				}
...
document.getElementById("buttonBuy").addEventListener("click", onPointerDown);
function onPointerDown() {
    window.location.pathname = '/halamanBaru';
}
